...
try {
	...
	do {
		Start-Sleep 1;
		$serverStatus = docker inspect --format='{{.State.Health.Status}}' my2000thapp_server_1
		...
		$clientStatus = docker inspect --format='{{.State.Health.Status}}' my2000thapp_client_1;
	...
	} Until((docker-compose --project-name "my2000thapp" ps | Select-String 'Up \(healthy\)' | Measure-Object -Line).Lines -gt 1)
